...
public class TooltipFormatterTests {
  [Fact]
  public void No_active_recordings_shows_idle() {
    var text = TooltipFormatter.Format([]);

    Assert.Equal("Xtreamium — idle", text);
  }
  ...
  public void Single_recording_without_progress_shows_title_only() {
    ...
    var text = TooltipFormatter.Format(active);

    Assert.Equal("Recording: Show A", text);
  }
  ...
  public void Single_recording_with_progress_shows_elapsed_and_duration_in_minutes() {
    ...
    Assert.Equal("Recording: Show A (12m / 60m)", text);
  }
  ...
  public void Multiple_recordings_are_listed_one_per_line_with_a_header() {
    ...
    Assert.Equal("Recording 2 shows:\nShow A (12m / 60m)\nShow B", text);
  }
  ...
  public void Minutes_are_truncated_not_rounded() {
    ...
    Assert.Equal("Recording: Show A (1m / 60m)", text);
  }
}
